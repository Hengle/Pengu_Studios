        ...
        void GetMaterialConversionFromPreset(Material material)
        {
            for (int i = 0; i < presetLines.Count; i++)
            {
                if (presetLines[i].StartsWith("Material"))
                {
                    ...
                    else if (type == "MATERIAL_NAME_TO_SHADER_FIND")
                    {
                        if (material.shader.name.Contains(src))
                        {
                            var shader = presetLines[i].Replace("Material MATERIAL_NAME_TO_SHADER_FIND " + src + " ", "");
                    ...
                    else if (type == "SHADER_NULL_TO_SHADER_FIND")
                    {
                        if (material.shader == null)
                        {
                            var shader = presetLines[i].Replace("Material SHADER_NULL_TO_SHADER_FIND ", "");
