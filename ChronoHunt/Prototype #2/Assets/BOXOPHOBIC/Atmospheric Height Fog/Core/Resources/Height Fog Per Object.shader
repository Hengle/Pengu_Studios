	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag ( VertexOutput IN  ) : SV_Target
			{
				...
				half FogDistanceMask12_g781 = pow( abs( saturate( ( ( distance( WorldPosition2_g781 , _WorldSpaceCameraPos ) - temp_output_7_0_g860 ) / ( AHF_FogDistanceEnd - temp_output_7_0_g860 ) ) ) ) , AHF_FogDistanceFalloff );
				float3 lerpResult258_g781 = lerp( (AHF_FogColorStart).rgb , (AHF_FogColorEnd).rgb , ( saturate( ( FogDistanceMask12_g781 - 0.5 ) ) * AHF_FogColorDuo ));
				float3 normalizeResult318_g781 = normalize( ( WorldPosition2_g781 - _WorldSpaceCameraPos ) );
				float dotResult145_g781 = dot( normalizeResult318_g781 , AHF_DirectionalDir );
				half DirectionalMask30_g781 = pow( abs( ( (dotResult145_g781*0.5 + 0.5) * AHF_DirectionalIntensity ) ) , AHF_DirectionalFalloff );
				float3 lerpResult40_g781 = lerp( lerpResult258_g781 , (AHF_DirectionalColor).rgb , DirectionalMask30_g781);
				float3 temp_output_2_0_g859 = lerpResult40_g781;
				float3 gammaToLinear3_g859 = FastSRGBToLinear( temp_output_2_0_g859 );
				#ifdef UNITY_COLORSPACE_GAMMA
				float3 staticSwitch1_g859 = temp_output_2_0_g859;
				#else
				float3 staticSwitch1_g859 = gammaToLinear3_g859;
				#endif
				float3 temp_output_256_0_g781 = staticSwitch1_g859;
				...
				half FogHeightMask16_g781 = pow( abs( saturate( ( ( ( break159_g781.x + break159_g781.y + break159_g781.z ) - temp_output_7_0_g861 ) / ( AHF_FogHeightStart - temp_output_7_0_g861 ) ) ) ) , AHF_FogHeightFalloff );
				float lerpResult328_g781 = lerp( ( FogDistanceMask12_g781 * FogHeightMask16_g781 ) , saturate( ( FogDistanceMask12_g781 + FogHeightMask16_g781 ) ) , AHF_FogLayersMode);
				...
				half NoiseDistanceMask7_g781 = saturate( ( ( distance( WorldPosition2_g781 , _WorldSpaceCameraPos ) - temp_output_7_0_g863 ) / ( 0.0 - temp_output_7_0_g863 ) ) );
				...
				float staticSwitch42_g781 = lerpResult328_g781;
				#elif defined(AHF_NOISEMODE_PROCEDURAL3D)
				float staticSwitch42_g781 = ( lerpResult328_g781 * NoiseSimplex3D24_g781 );
				#else
				float staticSwitch42_g781 = lerpResult328_g781;
				#endif
				float temp_output_43_0_g781 = ( staticSwitch42_g781 * AHF_FogIntensity );
				float2 uv_MainTex = IN.ase_texcoord3.xy * _MainTex_ST.xy + _MainTex_ST.zw;
				float temp_output_1_0_g753 = tex2Dbias( _MainTex, float4( uv_MainTex, 0, 0.0) ).a;
				...
				float3 Color = temp_output_256_0_g781;
...
Node;AmplifyShaderEditor.SamplerNode;892;-3072,-3840;Inherit;True;Property;_MainTex;MainTex;10;0;Create;False;0;0;False;0;False;-1;None;None;True;0;False;white;Auto;False;Object;-1;MipBias;Texture2D;8;0;SAMPLER2D;;False;1;FLOAT2;0,0;False;2;FLOAT;0;False;3;FLOAT2;0,0;False;4;FLOAT2;0,0;False;5;FLOAT;1;False;6;FLOAT;0;False;7;SAMPLERSTATE;;False;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
